...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        def dev_step(x_batch, y_batch, writer=None):
            ...
            acc = 0.0
            feat = np.zeros((len(x_batch), len(map(int, FLAGS.filter_sizes.split(",")))*FLAGS.num_filters));
            N = len(x_batch)/Ntest
            for i in range(N):            
